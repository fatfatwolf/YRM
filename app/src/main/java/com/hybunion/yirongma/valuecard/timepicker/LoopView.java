...
public class LoopView extends View {
    ...
    boolean isViewYear=true;

    public void setIsViewYear(boolean isviewYear){
        isViewYear=isviewYear;
    }
    ...
    public final void setList(List<String> arraylist) {
        if(isViewYear){
            arrayList=arraylist;
        }else{
            arrayList = new ArrayList<>();
            for (int i = 0; i < arraylist.size(); i++) {
                if (arraylist.get(i).contains("年")) {
                    arrayList.add(arraylist.get(i).substring(arraylist.get(i).indexOf("年") + 1, arraylist.get(i).length()));
                } else {
