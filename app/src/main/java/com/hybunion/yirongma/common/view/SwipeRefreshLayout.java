...
//    private boolean mIsBootom = true;
    ...
    public void setColorSchemeColors(int color1, int color2, int color3, int color4) {
        Log.d("find", "setColorSchemeColors");
        ensureTarget();
...
//                    }
    ...
    private void setDefaultHeadViewRefreshing() {
        if (mIsDefaultHeadView) {
            ...
            mRefreshTitle.setText(getResources().getString(R.string.refreshing));
        }
    }

    private void updateContentOffsetTop(int targetTop) {
        final int currentTop = mTarget.getTop();
        Log.d("find", "currentTop==" + currentTop);
...
//        } else
        ...
        if (currentTop > mDistanceToTriggerSync) {
            Log.d("find", "mCanRefreshing==" + mCanRefreshing + "," + mCanRefreshing);
            ...
            Log.d("find", "mCanRefreshing2==" + mIsPulling + "," + mRefreshing);
            if (!mIsPulling && !mRefreshing) {
                mCanRefreshing = false;
                if (mPullListener != null) {
                    mPullListener.onPulling(mHeadView, currentTop);
                }
                if (mIsDefaultHeadView) {
                    mRefreshImageview.clearAnimation();
