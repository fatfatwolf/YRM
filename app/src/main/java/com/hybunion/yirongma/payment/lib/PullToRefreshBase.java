...
import com.hybunion.yirongma.payment.lib.internal.BaseLoadingLayout;
import com.hybunion.yirongma.payment.lib.internal.LoadingLayout;
import com.hybunion.yirongma.payment.lib.internal.SpecialTopLoadingLayout;
import com.hybunion.yirongma.payment.lib.internal.SpecialTopNoPicUpAndDown;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	static final int RELEASE_TO_REFRESH_DOWN = 0x1;
	static final int RELEASE_TO_REFRESH_UP = 0x4;
	...
	private OnRefreshListener onDownPullRefreshListener;
	private OnRefreshListener onUpPullRefreshListener;
	...
	/**
	 * Get the Wrapped Refreshable View. Anything returned here has already beenLazyScrollView
	 ...
	 */
	public final void setOnDownPullRefreshListener(OnRefreshListener listener) {
		onDownPullRefreshListener = listener;
	}
	 ...
	 *            - Listener to be used when the Widget is set to Refresh
	 */
	public final void setOnUpPullRefreshListener(OnRefreshListener listener) {
		onUpPullRefreshListener = listener;
	}
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_UP: {
				if (isBeingDragged) {
					...
					if (state == RELEASE_TO_REFRESH_DOWN && null != onDownPullRefreshListener) {
						
						setRefreshingInternal(true);
						onDownPullRefreshListener.onRefresh();
					}else if(state == RELEASE_TO_REFRESH_UP && null != onUpPullRefreshListener) {
						
						setRefreshingInternal(true);
						onUpPullRefreshListener.onRefresh();
					}else{
						smoothScrollTo(0);
					}
					return true;
	...
	private void init(Context context, AttributeSet attrs) {
		...
		String pullLabel = context.getString(R.string.pull_to_refresh_pull_label);//pull之前
		String pullLabel1="";

		String refreshingLabel = context.getString(R.string.pull_to_refresh_refreshing_label);//加载时。。。

		String releaseLabel = context.getString(R.string.pull_to_refresh_release_label);//准备释放时  上下两行字
		String releaseLabel1 ="";

		// Add Loading Views
		if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) {
			//System.out.println("refreshableView.getClass().getSimpleName()============"+refreshableView.getClass().getSimpleName());
			/*if(refreshableView.getClass().getSimpleName().equals("ScrollView")){
				headerLayout = new SpecialTopLoadingLayout(context, MODE_BOTH, releaseLabel,releaseLabel1, pullLabel,pullLabel1,
						refreshingLabel);
			}else*/

			//System.out.println("==========refreshableView.getClass().getSimpleName()========"+refreshableView.getClass().getSimpleName());

			if(refreshableView.getClass().getSimpleName().equals("Pull_Nopic_Up_Down_ScrollView")){//下拉上面没有图片
				headerLayout = new SpecialTopNoPicUpAndDown(context, MODE_PULL_DOWN_TO_REFRESH, releaseLabel,releaseLabel1, pullLabel,pullLabel1,
						refreshingLabel);
			}else if(refreshableView.getClass().getSimpleName().equals("LazyScrollView")){//下拉有图片  把LazyScrollView换成GridView  推荐那用到的
				headerLayout = new SpecialTopLoadingLayout(context, MODE_PULL_DOWN_TO_REFRESH, releaseLabel,releaseLabel1, pullLabel,pullLabel1,
						refreshingLabel);
			}else{//下拉有箭头加载
				boolean isShow = false;
//				if(refreshableView.getClass().getSimpleName().equals(PullToRefreshGridView.InternalGridView.class.getSimpleName())){//推荐
//					isShow=true;
//				}else{
//					isShow=false;
//				}
				headerLayout = new LoadingLayout(context, MODE_PULL_DOWN_TO_REFRESH, releaseLabel,releaseLabel1, pullLabel,pullLabel1,
						refreshingLabel,isShow);
			}
		...
		if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
			footerLayout = new LoadingLayout(context, MODE_PULL_DOWN_TO_REFRESH, releaseLabel,releaseLabel1, pullLabel,pullLabel1,
					refreshingLabel,false);//下面向上拉要是用的话，正常加载：
		...
		if (a.hasValue(R.styleable.PullToRefresh_headerBackground_pull)) {
			this.setBackgroundResource(a.getResourceId(R.styleable.PullToRefresh_headerBackground_pull, Color.WHITE));
		}
...
//				newHeight = Math.round((initialMotionY - lastMotionY) / FRICTION);
		...
		if (newHeight != 0) {
			if (state == PULL_TO_REFRESH && headerHeight < Math.abs(newHeight)) {
				...
				switch (currentMode) {
					case MODE_PULL_UP_TO_REFRESH:
						state = RELEASE_TO_REFRESH_UP;
						footerLayout.releaseToRefresh();
						break;
					case MODE_PULL_DOWN_TO_REFRESH:
						
						state = RELEASE_TO_REFRESH_DOWN;
			...
			} else if ((state == RELEASE_TO_REFRESH_DOWN || state == RELEASE_TO_REFRESH_UP) && headerHeight >= Math.abs(newHeight)) {
				state = PULL_TO_REFRESH;
