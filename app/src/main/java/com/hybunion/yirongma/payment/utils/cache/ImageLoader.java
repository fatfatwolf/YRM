...
//			imageView.setImageResource(stub_id);
	...
	private Bitmap getBitmap(String url,int requiredWidth) {
		...
		try {
			...
			LogUtils.d("", "getBitmap catch Exception...\nmessage = " + ex.getMessage());
			return null;
		}
	}
	...
	private Bitmap decodeFile(File f,int requiredWidth) {
		try {
			...
			if(requiredWidth > 0){
				...
				if(scale<1)scale=1;
			}else if(width_tmp> CommonUtil.getScreenWidth(context)){
				scale = width_tmp / CommonUtil.getScreenWidth(context);
				if (scale < 1) scale = 1;
			}
            in1.close();
			...
			LogUtils.d(getClass().getName(), e.toString() + " " + e.getMessage());
		}
		return null;
	}
	...
	public static void CopyStream(InputStream is, OutputStream os) {
		...
		try {
			...
			LogUtils.d("", "CopyStream catch Exception...");
		}
	}
}