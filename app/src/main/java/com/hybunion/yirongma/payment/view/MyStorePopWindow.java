...
import com.hybunion.yirongma.payment.adapter.StoreTwiceListAdapter;
...
public class MyStorePopWindow extends PopupWindow {
    ...
    Button bt_sure;
    StoreTwiceListAdapter storeListAdapter;
    ...
    int multiple = 0;//多选模式 使用，0代表可取消，其他当choose为true时候不可取消
    int type = 0;

    public MyStorePopWindow(Activity context, List<StoreManageBean.ObjBean> storeList,int multiple) {
        super(context);
    ...
    public MyStorePopWindow(View contentView, int width, int height, Activity mContext) {
        ...
        listview_store.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (storeListener != null) {
                    ...
                    if(multiple == 0){//可以多选取消
                        storeList.get(position).isStoreChoose = !storeList.get(position).isStoreChoose;
                    }else {
                        if(storeList.get(position).isStoreChoose){
                            storeList.get(position).isStoreChoose2 = false;
                        }else {
                            storeList.get(position).isStoreChoose2 = !storeList.get(position).isStoreChoose2;
                        }
                    }
...
//                storeListAdapter.notifyDataSetChanged();
        ...
        if (storeList != null) {
            if (storeListAdapter == null) {
                storeListAdapter = new StoreTwiceListAdapter(mContext, storeList,multiple);
            }
            listview_store.setAdapter(storeListAdapter);
        }
    }
    ...
    public void showClerkList(List<QueryClerkListBean.ObjBean> list, OnKuanTaiItemListener kuanTaiListener) {
        ...
        if (!YrmUtils.isEmptyList(mClerkList)) {
            ...
            listview_kuantai.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                @Override
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    if (MyStorePopWindow.this.kuanTaiListener != null) {
                        MyStorePopWindow.this.kuanTaiListener.setKuanTaiItemListener(position);
                    }
